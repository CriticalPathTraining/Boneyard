...
namespace ProductsListPopulationUtility {
  ...
  class Program {
    ...
    static ListCollection lists = site.Lists;
    ...
    static void Main() {
      ...
      try {
        ...
        listProducts = lists.GetByTitle("Products");
        clientContext.Load(listProducts);

        DeleteExitingProducts();
        ...
        Console.WriteLine();
        Console.WriteLine("---------------------------------------------------------");
        Console.WriteLine("--  There was an error attempting to load Product data --");
        Console.WriteLine("---------------------------------------------------------");
        Console.WriteLine();
    ...
    static void DeleteExitingProducts() {
      // delete any existing products
      ListItemCollection existingProducts = listProducts.GetItems(new CamlQuery());
      clientContext.Load(existingProducts);
      clientContext.ExecuteQuery();

      if (existingProducts.Count > 0) {
        Console.WriteLine("Deleting existing products");
        for (int i = (existingProducts.Count - 1); i >= 0; i--) {
          existingProducts[i].DeleteObject();
          clientContext.ExecuteQuery();
        }
      }
    }
    ...
    static void PopulateProductsList() {
      ...
      Console.WriteLine();
      Console.WriteLine("Adding Wingtip Product items to Products list");
      Console.WriteLine();
      ...
      Console.WriteLine();
      Console.WriteLine("  All products have been added to Products list.");
